...
> **NOTE: エラーが出たら。** デプロイに限らず、このハンズオンでエラーが出たときの対処方法は一つです——**エラーメッセージ全文を、そのままエージェントに貼って直させる**。エージェントの生成コードは毎回少しずつ違うため、あなたの環境でだけ起きるエラーもあります（実際、このハンズオンの試走でも初回デプロイが IAM 権限エラーで失敗し、エラー全文を貼ることで解決しています）。直ったら同じコマンドを再実行するだけです。
